...
function buildPatientFigure() {
  ...
  ensureLayer('vascular', () => { if (activeLayer === 'vascular') applyLayer(); });  // Scene 3 default layer (vascular) streams in
}
...
onModeChange((mode, focusId) => {
  ...
  swapTimer = setTimeout(() => {
    ...
    if (mode === 'patient') {
      activeLayer = 'vascular'; applyLayer(); configurePatient(bedById(focusId)); // Scene 3 opens on the vascular system
