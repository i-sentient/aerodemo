...
const SHOTS: Shot[] = [
  { pos: [0, 6.0, -12], look: [0, 1.1, 3] },        // 0 · arrive INSIDE — wide interior (continues the intro film's zoom; no outside long-shot)
...
// On arrival the camera auto-flies from the establishing shot up to this beat
// (the inbound alert) with no keypresses; the presenter drives every beat after.
const AUTO_TO = 2
const AUTO_HOLDS = [2000, 2100] // ms held on beat 0, then beat 1, before advancing

export function RoundERLab({ onExit }: { onExit?: () => void } = {}) {
  const [step, setStep] = useState(0)
  // keep the latest step readable inside the (once-attached) key handler so we
  // can tell "advance a beat" from "already on the last beat → hand off".
  const stepRef = useRef(0)
  useEffect(() => {
    stepRef.current = step
  }, [step])
  // flips true the moment the presenter touches a key — cancels the auto fly-in
  // below so it never fights a manual keypress.
  const manual = useRef(false)
  // One-time cinematic auto-advance on arrival: the camera walks itself from the
  // establishing shot (0) up to the inbound alert (AUTO_TO) with no keypresses,
  // so it flows straight out of the intro film's zoom. Only ever moves FORWARD,
  // and stops the instant the presenter takes over.
  useEffect(() => {
    const timers: number[] = []
    let t = 0
    for (let target = 1; target <= AUTO_TO; target++) {
      t += AUTO_HOLDS[target - 1]
      timers.push(
        window.setTimeout(() => {
          if (!manual.current) setStep((s) => (s < target ? target : s))
        }, t),
      )
    }
  ...
  // Pressing → / Space again while already on the final beat (TARS → ICU) fires
  // onExit — the parent then fades to black and swaps in the ICU ward.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.code === 'Space' || e.code === 'ArrowRight') {
        e.preventDefault()
        manual.current = true
        if (stepRef.current >= N_BEATS - 1) {
          onExit?.()
          return
        }
  ...
  if (import.meta.env.DEV) (window as unknown as Record<string, unknown>).__er = { step }
  return (
    <div style={{ position: 'fixed', inset: 0 }}>
      <Canvas
        ...
        camera={{ position: [0, 6.0, -12], fov: 55 }}
      >
