...
const dockApps = () => (state.mode === 'patient' ? ['emr', 'lis', 'pacs'] : ['emar', 'inventory', 'staffing', 'protocols']);
...
export function initApps() {
  ...
  setInterval(() => {
    if (state.mode !== 'patient' || activeApp !== 'emr') return;
...
export function openApp(k) {
  ...
  if (k === 'emr') {
    ...
    if (lastFocus && lastFocus.section === emrSection) requestAnimationFrame(() => focusField(lastFocus.field, lastFocus.label, false));
  }
}
